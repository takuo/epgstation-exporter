...
func (c *Collector) collectRules(ctx context.Context, ch chan<- prometheus.Metric) error {
	// 生成コードの GetRulesParams.Type が interface{} 型のため nil パニックが発生する問題を回避し、
	// 全件取得のページネーションも行うため直接 HTTP 呼び出しを使う。
	var rulesExt epgstation.RulesExtended
	limit := 500
	offset := 0
	for {
		reqURL := fmt.Sprintf("%s/rules?limit=%d&offset=%d", c.apiURL, limit, offset)
